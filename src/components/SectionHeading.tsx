 ...
 * expression, but it stays a short rise rather than a per-letter cascade,
 * which reads as showing off at this size. No blur: blurring display type is
 * a per-frame re-raster of a large text layer, not a composited property, and
 * this runs on every heading on the page.
 *
...
export function SectionHeading({
  ...
  // Built inside the component: a variant's own transition wins over the
  // component's `transition` prop, so a module-level staggerChildren would
  // survive prefers-reduced-motion and step the words in one at a time anyway.
  const container: Variants = {
    hidden: {},
    shown: { transition: { staggerChildren: reduce ? 0 : 0.07 } },
  };
